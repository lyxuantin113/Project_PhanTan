...
//	Nút Chuyển Qua DS Hóa Đơn ????????????????
	...
	public DSDonDat_Gui() throws RemoteException {
//		JPANEL
...
//		End
		...
		donDatDao = new DonDat_Impl();
		ctddDao = new ChiTietDonDat_Impl();
	...
	private void hoanThanhDon() throws RemoteException {
		...
		if (rowSelected != -1) {
			...
			if (choice == JOptionPane.YES_OPTION) {
				...
				hoaDon.setListChiTiet(listCTHD);
			...
			if (typeSearch.equalsIgnoreCase("Mã đơn")) {
				...
				if (listHD != null) {
					...
					for (DonDat donDat : listHD) {
						Object[] rowData = { donDat.getMaDonDat(), donDat.getMaNhanVien().getMaNhanVien(),
								donDat.getMaKhachHang().getTenKhachHang(), donDat.getMaKhachHang().getSoDienThoai(), donDat.getNgayLap(),
